...
- `loader.py` — YAML definition loading (imported lazily; requires PyYAML). Reserved keys `factory`/`scope`/`provides`/`default`/`eager`/`profiles`/`params`; every other key is a constructor param (`name` deliberately included — some files pass it to the factory). `!ENV` substitutes `${VAR}`/`${VAR:-fallback}` at load time (strings only, never re-parsed), `!ref` injects other services at build time, mappings containing `factory` become anonymous fresh instances, `.`-prefixed top-level keys are anchor-only templates.
